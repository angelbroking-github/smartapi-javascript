...
var SmartApi = function (params) {
    ...
    /**
     * find instrument details from instrument list by Name
     * @param {string} symbol
     */
    self.getInstrumentByName = function (symbol) {
      if (!self.instrument_list.length) {
        
        return new Promise((resolve) => {
          self.load_instrument_list().then(()=>{
            resolve(self.instrument_list.get(symbol));
          })
        });
      } else {
        return new Promise((resolve) => {
    ...
    /**
     * load all instrument details 
     * it take so much time to load because 70k+ records, user has to wait
     */
    self.load_instrument_list = function () {
        const agent = new https.Agent({
            rejectUnauthorized: false,
          });
      return axios
        .get(API.open + API.get_all_instrument_list, { httpsAgent: agent })
        .then((result) => {
          result.data.forEach((element) => {
            self.instrument_list.set(element.symbol, element);
          });
        })
